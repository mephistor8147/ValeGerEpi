...
export function Exchange({ onBack, adminUser }: ExchangeProps) {
  ...
  const episByCategory = filteredEpis.reduce(
    (acc, epi) => {
  ...
  return (
    <div className="flex flex-col flex-1 bg-[#0D2027] h-full">
      ...
      <div className="bg-[#152A32] px-6 pt-16 md:pt-12 pb-10 rounded-b-[40px] md:rounded-b-[50px] relative overflow-hidden bg-cover bg-center bg-no-repeat flex items-center justify-between text-white shrink-0 shadow-md"
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-[#FFA767] tracking-tight drop-shadow-sm">Troca de EPI</h1>
        <div className="w-8"></div>
      </div>

      <div className="flex-1 flex flex-col overflow-hidden max-w-4xl mx-auto w-full">
        {/* Stepper */}
        <div className="bg-[#152A32] px-4 md:px-12 py-8 border-b border-[#2C4550] shrink-0 shadow-sm md:rounded-b-3xl">
          <div className="flex justify-between relative max-w-lg mx-auto w-full">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <span
                className={cn(
                  "text-xs md:text-sm font-bold text-center",
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <span
                className={cn(
                  "text-xs md:text-sm font-bold text-center",
                  step >= 4 ? "text-[#FFA767]" : "text-[#475569]",
                )}
              >
                Resumo
              </span>
            ...
            <div className="w-full max-w-4xl">
                ...
                <div className="w-full max-w-2xl mx-auto">
                  <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <h2 className="text-3xl md:text-4xl font-bold text-[#FFA767] tracking-tight drop-shadow-sm text-[#E2E8F0]">
                      Selecione EPIs que serão devolvidos
                    </h2>
                  </div>
              ...
              {step === 3 && (
                <div className="w-full">
                  <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <h2 className="text-3xl md:text-4xl font-bold text-[#FFA767] tracking-tight drop-shadow-sm text-[#E2E8F0]">
                      Selecione EPIs que serão entregues
                    </h2>
                    <div className="relative max-w-sm w-full">
                      ...
                      <input
                        ...
                        className="w-full pl-11 pr-4 py-3 bg-[#152A32] border border-[#2C4550] rounded-full focus:ring-2 focus:ring-[#FFA767] outline-none"
                      />
                    </div>
                  </div>

                  <div className="space-y-8">
                    {(Object.entries(episByCategory) as [string, any[]][])
                      .sort()
                      .map(([category, items]) => (
                        <div key={category}>
                          <h3 className="text-lg font-bold text-[#FFA767] border-b border-[#2C4550] pb-2 mb-4">
                            {category}
                          </h3>
                            ...
                            {items.map((epi) => {
                              ...
                              return (
                                <div
                                  key={epi.id}
                                  onClick={() =>
                                    !outOfStock && toggleEpiSelection(epi.id)
                                  }
                                  className={cn(
                                    "flex items-center p-4 rounded-xl border-2 cursor-pointer transition-all gap-4",
                                    isSelected
                                      ? "border-[#FFA767] bg-[#152A32] shadow-sm"
                                      : outOfStock
                                        ? "border-[#2C4550] bg-[#0D2027] opacity-60 cursor-not-allowed"
                                        : "border-[#253B44] bg-[#152A32] hover:border-[#FFA767]/30",
                                  )}
                                >
                                  <div
                                  ...
                                  <div className="flex-1">
                                    <p
                                      className="font-semibold text-[#E2E8F0] line-clamp-1"
                                      title={epi.nome}
                                    >
                                      {epi.nome}
                                    </p>
                  ...
                  <div className="bg-[#152A32] border text-left border-[#2C4550] rounded-2xl p-6 shadow-sm mb-6 space-y-6">
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                          <p className="text-xs text-[#64748B] mb-1">Data</p>
                          <p className="font-semibold text-[#E2E8F0]">
                    ...
                    <div className="border-t border-[#253B44] pt-6">
                      <h3 className="text-sm font-bold text-[#475569] uppercase tracking-wider mb-4">
                        EPIs Novas Entregas ({selectedEpis.length})
                      </h3>
                      <div className="space-y-3">
                        {selectedEpis.map((epiId) => {
                          ...
                          return epi ? (
                            <div
                              key={epi.id}
                              className="flex justify-between items-center p-3 bg-[#152A32] rounded-xl border border-[#253B44]"
                            >
                              <div>
                              ...
                              <div className="text-xs font-bold text-green-700 bg-[#253B44] px-2 py-1 rounded">
                                + Novo
                              </div>
                            </div>
                          ) : null;
