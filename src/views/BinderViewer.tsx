...
import { collection, doc, query, where, orderBy, onSnapshot, getDocs } from 'firebase/firestore';
...
export function BinderViewer({ employeeId, onBack }: BinderViewerProps) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const cargos = cargosSnap.docs.reduce((acc, d) => ({ ...acc, [d.id]: d.data().titulo }), {} as Record<string, string>);
        
        const obrasSnap = await getDocs(collection(db, 'obras'));
        const obras = obrasSnap.docs.reduce((acc, d) => ({ ...acc, [d.id]: d.data().nome }), {} as Record<string, string>);

        unsubEmployee = onSnapshot(doc(db, 'funcionarios', employeeId), (snap) => {
           if (snap.exists()) {
  ...
  return (
    <div className="flex flex-col flex-1 bg-gray-50 h-full">
      {/* Header */}
      <div className="bg-[#0B5C36] px-4 pt-12 md:pt-8 pb-6 flex items-center justify-between text-white shrink-0 shadow-md print:hidden">
        <button onClick={onBack} className="p-2 -ml-2 hover:bg-white/10 rounded-full transition-colors"><ChevronLeft size={24} /></button>
        <h1 className="text-xl md:text-2xl font-bold">Visualizar Fichário</h1>
        <div className="w-8"></div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-24 print:p-0 print:overflow-visible">
        ...
        {/* A4 Document Preview */}
        <div className="bg-white w-full max-w-[800px] mx-auto shadow-xl border border-gray-200 p-6 sm:p-10 text-[10px] sm:text-xs text-black font-sans relative print:shadow-none print:border-none print:m-0 print:max-w-none print:w-full print:p-0">
            ...
            <div className="flex flex-col sm:flex-row justify-around items-center sm:items-end pt-4 pb-2 mt-8 gap-8 sm:gap-0">
                ...
                <span className="mt-1 font-medium text-center">Assinatura do Colaborador</span>
                </div>
            </div>
            </div>
            ...
            <div className="flex border-b-2 border-black bg-gray-50 font-bold items-center text-center">
                <div className="w-[15%] sm:w-[12%] py-2 border-r border-black text-[9px] sm:text-[10px]">CÓDIGO</div>
                <div className="hidden sm:block w-[12%] py-2 border-r border-black">UNI</div>
                <div className="w-[20%] sm:w-[15%] py-2 border-r border-black text-[9px] sm:text-[10px]">CA</div>
                <div className="w-[35%] sm:w-[31%] flex flex-col border-r border-black">
                <div className="border-b border-black py-0.5 text-[9px] sm:text-[10px]">DATA</div>
                <div className="flex w-full">
                    <div className="w-1/2 border-r border-black py-0.5 text-[8px] sm:text-[10px]">ENTREGA</div>
                    <div className="w-1/2 py-0.5 text-[8px] sm:text-[10px]">DEVOLUÇÃO</div>
                </div>
                </div>
                <div className="w-[30%] py-2 text-[9px] sm:text-[10px]">ASSINATURA</div>
            </div>

            {/* Linhas */}
            {Array.from({ length: Math.max(entries.length + 3, 12) }).map((_, i) => {
                const entry = entries[i];
                return (
                <div key={i} className="flex border-b border-black last:border-b-0 min-h-[28px]">
                    <div className="w-[15%] sm:w-[12%] border-r border-black flex items-center justify-center font-medium overflow-hidden px-1">{entry?.codigo || ''}</div>
                    <div className="hidden sm:flex w-[12%] border-r border-black items-center justify-center font-medium">{entry?.uni || ''}</div>
                    <div className="w-[20%] sm:w-[15%] border-r border-black flex items-center justify-center font-medium px-1">{entry?.ca || ''}</div>
                    <div className="w-[17.5%] sm:w-[15.5%] border-r border-black flex items-center justify-center font-medium">{entry?.dataEntrega || ''}</div>
                    <div className="w-[17.5%] sm:w-[15.5%] border-r border-black flex items-center justify-center font-medium">{entry?.dataDevolucao || ''}</div>
