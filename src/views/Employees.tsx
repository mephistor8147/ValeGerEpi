...
export function Employees({ onBack, onSelectEmployee }: EmployeesProps) {
  ...
  const [filterType, setFilterType] = useState<"Todos" | "Ativos" | "Inativos">(
    "Todos",
  );
  ...
  return (
    <div className="flex flex-col flex-1 bg-[#0D2027] h-full">
      ...
      <div className="bg-[#152A32] px-6 pt-16 md:pt-12 pb-10 rounded-b-[40px] md:rounded-b-[50px] relative overflow-hidden bg-cover bg-center bg-no-repeat flex items-center justify-between text-white shrink-0 shadow-md"
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-[#FFA767] tracking-tight drop-shadow-sm">Funcionários</h1>
      ...
      <div className="p-4 md:p-8 flex flex-col flex-1 overflow-hidden max-w-5xl mx-auto w-full">
        ...
        <div className="relative mb-6 shrink-0">
          ...
          <input
            ...
            className="w-full bg-[#152A32] border border-[#2C4550] rounded-2xl py-4 pl-14 pr-4 text-base shadow-sm focus:ring-2 focus:ring-[#FFA767] outline-none transition-shadow hover:shadow-md"
          />
        </div>
        ...
        <div className="flex gap-2 md:gap-4 mb-6 shrink-0 overflow-x-auto pb-2 scrollbar-hide">
          <button
            ...
            className={cn(
              "px-6 py-2 rounded-full text-base md:text-lg font-bold whitespace-nowrap transition-colors",
              filterType === "Todos"
                ? "bg-[#FFA767] text-white"
                : "bg-[#152A32] text-[#94A3B8] border border-[#2C4550] hover:bg-[#0D2027]",
            )}
          >
            Todos
          </button>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 md:space-y-4 pb-safe pb-8 pr-2">
          {filteredEmployees.map((emp) => (
            <button
              ...
              className="w-full bg-[#152A32] p-4 md:p-5 rounded-2xl shadow-sm hover:shadow-md flex items-center justify-between border border-[#253B44] transition-all hover:border-[#FFA767]/30 group text-left"
            >
              <div className="flex items-center gap-4 md:gap-6 flex-1 min-w-0">
                <div className="w-14 h-14 md:w-16 md:h-16 flex items-center justify-center bg-[#0D2027] rounded-full border-2 border-[#253B44] group-hover:border-[#253B44] transition-colors overflow-hidden shrink-0">
                  {emp.fotoUrl ? (
                    <img
                      src={emp.fotoUrl}
                      alt={emp.nome}
                ...
                <div className="flex-1 min-w-0 pr-2">
                  <h3 className="font-bold text-[#E2E8F0] md:text-lg truncate">
                    {emp.nome}
                  </h3>
                  <p className="text-sm text-[#64748B] mb-0.5 truncate">
                    {cargos[emp.cargoId as keyof typeof cargos] || "S/ Cargo"}
                  </p>
              ...
              <div className="flex items-center gap-2 md:gap-4 shrink-0">
                ...
                <ChevronRight
                  size={20}
                  className="text-[#475569] group-hover:text-[#FFA767] transition-colors shrink-0 hidden sm:block"
                />
              </div>
            </button>
          ))}
